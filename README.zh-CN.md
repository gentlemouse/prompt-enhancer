...
| 复杂请求 | 套用同一套固定模板 | 从 5 套策略中选最合适的，基于 15+ 维度信号 |
| 你精心写好的结构化提示词 | 照样重写 | 识别已有结构，只做精准微调 |
| 追问，如「能再详细点吗？」 | 当作全新问题处理 | 识别为追问，解析模糊指代，保持对话感 |
| 修正补充，如「再加上异步处理」 | 忽略上下文 | 将新约束融入原始请求，输出完整版本 |
...
| **复杂度** | 思维链信号、多重问题、反思标记 |
| **上下文** | 新话题 / 追问 / 修正补充 — 基于 5 轮会话记忆 |
| **结构** | 是否已包含角色/任务/约束等结构 |
| **语言** | 中文或英文 — 全程保持 |

### 第二阶段 · 策略选择引擎

基于分析结果，从 **5 套策略**中选择最合适的一个：
...
|---|---|
| **优化前** | `那个能再详细点吗` |
| **优化后** | `请对上述方案中「数据存储架构」部分进行更详细的展开，重点对比 SQL 和 NoSQL 在此场景下的优劣，并给出推荐方案。` |
